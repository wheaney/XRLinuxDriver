...
connection_t* connection_pool_find_driver_connection(const char* driver_id, int* remove_index) {
...
void connection_pool_handle_device_added(const device_driver_type* driver, device_properties_type* device) {
    ...
    // don't do anything if this driver connection is already being tracked
    int index = -1;
    connection_t* c = connection_pool_find_driver_connection(driver->id, &index);
    if (!c) {
        ensure_capacity(pool);
        c = (connection_t*)calloc(1, sizeof(*c));
        c->driver = driver;
...
void connection_pool_handle_device_removed(const char* driver_id) {
    ...
    int remove_index = -1;
    connection_t* c = connection_pool_find_driver_connection(driver_id, &remove_index);
    if (c) {
