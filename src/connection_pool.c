...
void connection_pool_ingest_imu_quat(const char* driver_id, uint32_t timestamp_ms, imu_quat_type quat) {
    pthread_mutex_lock(&pool->mutex);
    ...
    if (conn) {
        conn->last_quat = quat;
        conn->last_ts_ms = timestamp_ms;
        conn->have_last = true;
    }

    // Update rate estimators first
    if (idx == primary_idx) {
        imu_rate_estimator_add(pool->rate_est_primary, timestamp_ms);
    } else if (idx == supp_idx) {
        imu_rate_estimator_add(pool->rate_est_supp, timestamp_ms);
    }
    ...
    if (pool->time_sync_initialized && pool->time_sync) {
        ...
        if (src >= 0) {
            ...
            imu_quat_type rel = quat;
            if (c && c->ref_set) {
                imu_quat_type conj = conjugate(c->ref_quat);
                rel = multiply_quaternions(conj, quat);
            }
            c->last_rel_quat = rel;
    ...
    imu_quat_type fused_for_forward = quat;
    if (should_forward) {
    ...
    if (should_forward) {
        extern void driver_handle_imu_event(const char* driver_id, uint32_t timestamp_ms, imu_quat_type quat);
        driver_handle_imu_event(driver_id, timestamp_ms, fused_for_forward);
    }
}
