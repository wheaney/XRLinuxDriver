...
static const device_properties_type viture_legacy_properties = {
    .brand                              = "VITURE",
    .model                              = "One",
...
static const int frequency_enum_to_value[] = {
    [IMU_FREQUENCE_60] = 60,
    [IMU_FREQUENCE_90] = 90,
...
static const char* frequency_to_string[] = {
    ...
    if (vendor_id == VITURE_ID_VENDOR) {
        for (int i=0; i < VITURE_ID_PRODUCT_COUNT; i++) {
            if (product_id == viture_supported_id_product[i]) {
                ...
                *device = viture_legacy_properties;
...
const device_driver_type viture_legacy_driver = {
